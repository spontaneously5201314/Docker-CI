...
finalize_yum_repo ()
{
  ...
  yum install -y pygpgme --disablerepo='runner_gitlab-ci-multi-runner'
  ...
  yum install -y yum-utils --disablerepo='runner_gitlab-ci-multi-runner'
  ...
  echo "Generating yum cache for runner_gitlab-ci-multi-runner..."
  yum -q makecache -y --disablerepo='*' --enablerepo='runner_gitlab-ci-multi-runner'
}
...
main ()
{
  ...
  yum_repo_config_url="https://packages.gitlab.com/install/repositories/runner/gitlab-ci-multi-runner/config_file.repo?os=${os}&dist=${dist}&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/runner_gitlab-ci-multi-runner.repo
  else
    yum_repo_path=/etc/yum.repos.d/runner_gitlab-ci-multi-runner.repo
  fi
